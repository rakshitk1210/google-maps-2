...
const ROUTE_SOURCE_ID = 'carplay-jam-route'
const ROUTE_LAYER_ID = 'carplay-jam-route-line'
...
// Live navigation map for the CarPlay screen. Navigation is already active on
// load, so the route to Mount Rainier draws immediately in a close-up driving
// view. When the phone sends a cafe stop, routeTarget flips and the map eases
// out to frame the new, shorter route.
export function CarPlayMap({ joined, routeTarget }: CarPlayMapProps) {
  ...
  // Redraw the route when the shared target changes (the initial draw is done
  // by the load handler above, so skip the first run).
  useEffect(() => {
