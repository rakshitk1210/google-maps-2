...
import { BANNER_TOWARD, THEN_LABEL } from './tripData'
import { MOTION_EMPHASIZED, tokens } from './theme'

interface NavBannerProps {
  /** Road the drive is heading toward — from the selected place. */
  road: string
}

// Portrait instruction banner, design.md §6.13 (iteration 8's banner with the
// road name parameterized): --teal-banner card, radius 24, 16px side margins,
// top 58px, 40px white maneuver arrow. Flat (rule 0.10). The "Then ↱" chip
// hangs off the banner's bottom-left corner.
export function NavBanner({ road }: NavBannerProps) {
  return (
    <Box
      sx={{
        position: 'absolute',
