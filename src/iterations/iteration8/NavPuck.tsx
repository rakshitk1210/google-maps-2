...
// The active-nav location puck: Google-blue chevron on a soft translucent
// white disc, with the gentle blue halo pulse from the design.md §2 blue-dot.
// Per rule 0.10 this is a map element, so it carries the pin shadow. Rendered
// inside a center-anchored mapboxgl.Marker (map stays bearing 0, so the
// chevron's CSS rotation alone conveys the direction of travel).
export function NavPuck({ heading }: NavPuckProps) {
  return (
    ...
    >
      {/* Halo — pulses gently, matching iteration 7's puck */}
      <Box
        sx={{
          position: 'absolute',
          inset: 0,
